fit <- readRDS("Models/AllStrains70.rds")

test_runs <- setdiff(unique_runs, train_runs)


# Subset your data for a particular strain
run_to_predict <- "HV287"
single_run_data <- glu01_data %>% filter(Run == run_to_predict)

# Predict and visualize the fit
pred_data <- add_predicted_draws(single_run_data, fit)
...
graph <- ggplot(pred_summary, aes(x = Time)) +
  ...
  labs(x = "Time (Seconds)", y = "Optical Density", title = paste("Time series plot for strain: ", run_to_predict)) + 
  theme_bw()
